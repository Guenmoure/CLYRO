...
export async function POST(request: NextRequest) {
  try {
    ...
    const parsed = improvePromptSchema.safeParse(await request.json())
    if (!parsed.success) {
      return NextResponse.json({ error: 'Invalid request body', code: 'VALIDATION_ERROR' }, { status: 400 })
    }
    const res = await fetch(`${API_URL}/api/v1/generate/improve-prompt`, {
      method: 'POST',
      headers: {
    ...
    console.error('[improve-prompt proxy]', err)
