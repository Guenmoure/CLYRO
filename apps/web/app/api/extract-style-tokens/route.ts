...
export async function POST(request: NextRequest) {
  try {
    ...
    const parsed = extractStyleTokensSchema.safeParse(await request.json())
    if (!parsed.success) {
      return NextResponse.json({ error: 'Invalid request body', code: 'VALIDATION_ERROR' }, { status: 400 })
    }
    const res = await fetch(`${API_URL}/api/v1/generate/extract-style-tokens`, {
      method: 'POST',
      headers: {
    ...
    console.error('[extract-style-tokens proxy]', err)
