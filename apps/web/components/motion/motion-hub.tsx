...
const PIPELINE = [
  { key: 'storyboard', label: 'mh_analyzeBrief',  pct: 20  },
  { key: 'visuals',    label: 'mh_genVisuals', pct: 55  },
  { key: 'audio',      label: 'mh_voiceover',           pct: 72  },
  { key: 'assembly',   label: 'mh_motionAssembly',  pct: 90  },
  { key: 'done',       label: 'mh_videoReady',      pct: 100 },
]
  ...
  return (
    <div className="mt-6">
      <p className="font-mono text-[11px] uppercase tracking-widest text-[--text-muted] mb-3">
        {t('mh_storyboard')} — {scenes.length} {t('mh_scene')}{scenes.length > 1 ? 's' : ''}
      </p>
    ...
    <div className="flex flex-col items-center justify-center h-full gap-6 px-8 max-w-lg mx-auto overflow-y-auto py-8">
      <div className="w-full">
        ...
        <p className="text-[--text-muted] text-sm text-center mb-6">
          {!isDone && !isError && t('mh_estimatedTime')}
        </p>
        ...
        {isError && (
          <div className="bg-error/10 border border-error/30 rounded-xl p-4 text-error text-sm mb-4">
            {errorMessage ?? t('mh_errorOccurred')}
          </div>
        )}
...
function CreationForm({ onGenerated }: { onGenerated: (id: string, title: string) => void }) {
  ...
  async function handleLogoUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      setLogoUrl(url)
      toast.success(t('mh_logoImported'))
    } catch {
      toast.error(t('mh_logoError'))
    } finally {
      ...
      <div className="flex-1 px-8 py-6 space-y-7">
        ...
        <div>
          <div className="flex items-center gap-2 mb-3">
            ...
            <p className="font-mono text-[11px] uppercase tracking-widest text-[--text-muted]">{t('mh_bgMusic')}</p>
          </div>
        ...
        <div>
          <p className="font-mono text-[11px] uppercase tracking-widest text-[--text-muted] mb-3">{t('mh_brandIdentity')}</p>
          <div className="flex gap-3 flex-wrap">
            {/* Couleur principale */}
            <div>
              <label htmlFor="brand-color" className="font-mono text-[11px] text-[--text-muted] mb-1.5 block">{t('mh_primaryColor')}</label>
              <div className="flex items-center gap-2 bg-card border border-border rounded-xl px-3 py-2">
                <input id="brand-color" type="color" value={color} onChange={(e) => setColor(e.target.value)}
                  title={t('mh_primaryColor')}
                  aria-label={t('mh_primaryColor')}
            ...
            {/* Logo upload */}
            <div>
              <label className="font-mono text-[11px] text-[--text-muted] mb-1.5 block">{t('mh_logoOptional')}</label>
              <div className="flex items-center gap-2">
                {logoUrl ? (
