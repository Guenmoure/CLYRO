...
export function PlanBillingSection() {
  ...
  return (
    <div className="max-w-3xl space-y-8">
      <div>
        <h2 className="font-display text-2xl font-bold text-foreground">{t('plb_title')}</h2>
      ...
      <section className="space-y-3">
        <p className="font-body text-sm font-semibold text-foreground">{t('plb_your_plan')}</p>
          ...
          <div className="relative p-6 flex items-center justify-between gap-4 bg-card/70 backdrop-blur-sm">
            <div className="flex-1 min-w-0 space-y-2">
              ...
              <p className="font-body text-sm text-[--text-secondary] max-w-md">
                {isPaid ? t('plb_plan_paid_desc') : t('plb_plan_free_desc')}
              </p>
              <div className="inline-flex items-center gap-2 rounded-full bg-background/60 px-3 py-1 text-xs font-mono text-foreground">
                ...
                {plan.credits} {plan.credits !== 1 ? t('plb_credits_left') : t('plb_credit_left')}
              </div>
            </div>
            {!isPaid && (
              <Link
      ...
      <section className="space-y-3">
        <div className="flex items-center justify-between">
          <p className="font-body text-sm font-semibold text-foreground">{t('plb_addons')}</p>
          <button
            type="button"
            disabled
            title="Coming soon"
        ...
        <div className="flex items-center gap-3 rounded-xl border border-dashed border-border bg-muted/30 px-4 py-6">
          ...
          <p className="font-body text-sm text-[--text-secondary]">{t('plb_no_addons')}</p>
        </div>
      </section>

      {/* Billing history (stub) */}
      <section className="space-y-3">
        <p className="font-body text-sm font-semibold text-foreground">{t('plb_billing_history')}</p>
        <div className="rounded-xl border border-border bg-card px-4 py-8 text-center">
          <p className="font-body text-sm text-[--text-secondary]">{t('plb_no_invoices')}</p>
        </div>
      </section>
