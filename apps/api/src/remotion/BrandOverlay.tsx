...
// ── Style tokens ───────────────────────────────────────────────────────────────
// Chaque style Motion Design CLYRO a son identité visuelle propre

interface StyleTokens {
  overlayGradient: string      // gradient CSS sur l'image
  textColor: string
  accentColor: string
  fontWeight: number
  textSize: number             // px (base 1920×1080)
  letterSpacing: string
  accentBarHeight: number      // px barre décorative
  accentBarPosition: 'top' | 'bottom'
  backgroundFill: string       // couleur de fond quand pas d'image
  vignette: string             // vignette sombre sur les bords
}
...
const SceneFrame: React.FC<{ scene: Scene; brandConfig: BrandConfig }> = ({
  scene,
  brandConfig,
}) => {
  ...
  // Ken Burns — amplitude varie par style
  ...
  // Lignes diagonales style Dynamique
